...
define([
  ...
  var GAEventRequest = declare([GARequest], {
    ...
    /**
     * Looks at trackingEvents and decides if it should be logged.
     * @param {string} action Current action to log.
     */
    shouldRequest: function (action) {
      if (!this.active) return false;

      if (this.trackingEvents && this.trackingEvents.length > 0) {
        if (this.trackingEvents.includes(action.toUpperCase())) {
          return true;
        }
      }
      return false;
    ...
    requestGA: function (category, action, label, dimensions, metrics) {
      ...
      lang.mixin(data, metrics);

      if (this.validate(data, category, action, label)) {
        if (data[this.DIM_STATION] || data[this.DIM_MOUNT]) {
          //get broadcaster
          if (this.broadcasters[data[this.DIM_MOUNT]]) {
            ...
            this._getBroadcaster(data[this.DIM_MOUNT]).then(
              function (broadcaster) {
                //success
                self.broadcasters[data[self.DIM_MOUNT]] = broadcaster;
                var tmpObj = {};
