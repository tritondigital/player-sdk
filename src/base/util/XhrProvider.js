...
define(['dojo/_base/declare', 'dojo/request/xhr'], function (declare, xhr) {
  var xhrProvider = declare([], {
    constructor: function () {
      ...
      } else if (typeof XDomainRequest == 'object' || typeof XDomainRequest == 'function') {
        this.corsSupport = 'xd';
      } else {
        console.error('xhrProvider::constructor - CORS in not supported by the browser');
      }
    },

    request: function (url, requestData, requestArgs, successCallback, errorCallback) {
      ...
      if (this.corsSupport == 'xd') {
        ...
        xdr.onload = function () {
          if (requestArgs.handleAs.toLowerCase() == 'json') {
            if (successCallback != null && successCallback != undefined) successCallback(requestData, JSON.parse(xdr.responseText));
          } else {
            var response;
            if (window.DOMParser) {
            ...
            if (successCallback != null && successCallback != undefined) successCallback(requestData, response.documentElement || response);
          }
        };
        xdr.onprogress = function () {};
        xdr.ontimeout = function () {};
        xdr.onerror = function () {
          if (errorCallback != null && errorCallback != undefined) errorCallback(requestData, 'An unexpected error occurred');
        };
        ...
        xhr(url, requestArgs).then(
          function (data) {
            if (successCallback != null && successCallback != undefined) successCallback(requestData, data.documentElement || data);
          },
          function (error) {
            if (errorCallback != null && errorCallback != undefined) errorCallback(requestData, 'An unexpected error occurred: ' + error);
          }
        );
      } else {
        console.error('xhrProvider::request - CORS in not supported by the browser');
      }
    }
  });
