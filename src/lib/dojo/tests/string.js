define(['doh/main', '../string'], function (doh, string) {
  doh.register('tests.string', [
    ...
    function test_string_substitute(t) {
      t.is("File 'foo.html' is not found in directory '/temp'.", string.substitute("File '${0}' is not found in directory '${1}'.", ['foo.html', '/temp']));
      t.is(
        "File 'foo.html' is not found in directory '/temp'.",
        string.substitute("File '${name}' is not found in directory '${info.dir}'.", {
          name: 'foo.html',
          info: { dir: '/temp' }
        })
      );
    ...
    function test_string_substitute_transform(t) {
      ...
      t.is("file 'foo.html' is not found in directory '/temp'.", string.substitute('${0} is not found in ${1}.', ['foo.html', '/temp'], getPrefix));

      t.is("...file 'foo.html' is not found in ...directory '/temp'.", string.substitute('${0} is not found in ${1}.', ['foo.html', '/temp'], obj.getPrefix, obj));
    },

    function test_string_substitute_formatter(t) {
      t.is(
        'thinger -- howdy',
        string.substitute('${0:postfix}', ['thinger'], null, {
