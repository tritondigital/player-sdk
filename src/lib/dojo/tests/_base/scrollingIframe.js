...
function runScrollingTest(resultNode) {
  ...
  setTimeout(function () {
    var cw = dojo.hitch(dojo, "withGlobal")(window, "position", dojo, [
      clientWidth,
      false,
    ]);
    if (cw.x != 0) {
    ...
    var p = dojo.hitch(dojo, "withGlobal")(window, "position", dojo, [
      control,
      true,
    ]);
    ...
    setTimeout(function () {
      cw = dojo.hitch(dojo, "withGlobal")(window, "position", dojo, [
        clientWidth,
        false,
      ]);
      if (
        cw.x >= 0 ||
        (cw.x < 0 && Math.round(root.clientWidth - cw.w - cw.x) == 0)
      ) {
        ...
        resultNode.testResult =
          "100% width element start/size=" +
          cw.x +
          "/" +
          cw.w +
          " frame client left/width=" +
          root.clientLeft +
          "/" +
          root.clientWidth;
      }
...
function genScrollingTestNodes(hScroll, vScroll, large) {
  document.write(
    '<DIV id="abs1" style="position:absolute;background-color:red;left:0;top:0;width:1em;font-family:monospace;font-size:16px;">&nbsp;</DIV>' +
      ...
      (large
        ? (hScroll
            ? '<DIV style="float:left;position:relative;width:600px;">&nbsp;</DIV>'
            : "") +
          (hScroll
            ? '<DIV style="float:right;position:relative;width:600px;">&nbsp;</DIV>'
            : "") +
          (vScroll
            ? '<CENTER style="width:1px;height:600px;">&nbsp;</CENTER>'
            : "")
        : "") +
      ""
  );
...
function genScrollingTestBody() {
  ...
  document.write(
    '<BODY style="height:100%;margin:0;padding:0;border:0;background-color:white;overflow-x:' +
      (options.horz ? (isQuirks ? scroll : "") : "hidden") +
      ";overflow-y:" +
      (isQuirks ? (options.vert ? scroll : "hidden") : "") +
      ';">'
  );
  document.write(
    '<DIV id="clientWidth"><CENTER>' +
      (isQuirks ? "quirks " : "strict ") +
      (options.horz ? "horiz " : "") +
      (options.vert ? "vert " : "") +
      (options.large ? "scrolling " : "") +
      options.dir +
      "</CENTER></DIV>"
  );
...
if (!document.body) {
  ...
  document.write(
    '<DIV id="clientWidth" style="background-color:transparent;">&nbsp;</DIV>'
  );
