...
define([
  ...
  tests.data.readOnlyItemFileTestTemplates.registerTestsForDatastore(
    "dojo.data.ItemFileWriteStore"
  );
  ...
  doh.register("tests.data.ItemFileWriteStore", [
    ...
    function testWriteAPI_setValue() {
      ...
      var onComplete = function (items, request) {
        ...
        doh.assertTrue(store.isDirty());
        doh.assertEqual(
          store.getValue(item, "capital").toString(),
          "New Cairo"
        );
    ...
    function testWriteAPI_newItem_multiple_withParent() {
      ...
      function onComplete(items, request) {
        ...
        var newItem3 = store.newItem(
          { name: "Damanhur", abbr: "Damanhur" },
          { parent: item, attribute: "cities" }
        );
        deferred.callback(true);
      }
    ...
    function testWriteAPI_saveEverything() {
      ...
      store._saveEverything = function (
        ...
        doh.assertEqual(
          struct.identifier,
          store.getIdentityAttributes(egypt)[0]
        );
        ...
        var onItemClone = function (itemClone) {
          ...
          doh.assertEqual(
            store.getIdentityAttributes(egypt)[0],
            cloneStore.getIdentityAttributes(egyptClone)[0]
          );
          doh.assertEqual(
            store.getLabelAttributes(egypt)[0],
            cloneStore.getLabelAttributes(egyptClone)[0]
          );
          doh.assertEqual(
            store.getValue(egypt, "name"),
            cloneStore.getValue(egyptClone, "name")
          );
        };
    ...
    function testWriteAPI_saveEverything_HierarchyOff() {
      ...
      //		Simple test of the save API
      var store = new dojo.data.ItemFileWriteStore(
        tests.data.readOnlyItemFileTestTemplates.getTestData(
          "geography_hierarchy_small"
        )
      );
      ...
      store._saveEverything = function (
        ...
        var onItemClone = function (items, request) {
          ...
          doh.assertEqual(
            store.getValue(africa, "name"),
            cloneStore.getValue(africaClone, "name")
          );
        };
        cloneStore.fetch({
          query: { name: "Africa" },
      ...
      var intialFetch = function (items, request) {
        ...
        var afterNewFetch = function (items, request) {
          ...
          var afterDeleteFetch = function (items, request) {
            ...
            if (found) {
              deferred.errback(
                new Error("Error: Found the supposedly deleted item!")
              );
            } else {
              store.revert();
              ...
              var afterRevertFetch = function (items, request) {
                ...
                if (found) {
                  deferred.errback(
                    new Error("Error: Found the 'new' item after revert!")
                  );
                } else {
                  deferred.callback(true);
                }
    ...
    function testReferenceIntegrity_checkReferences() {
      ...
      var store = new dojo.data.ItemFileWriteStore(
        tests.data.ItemFileWriteStore.getTestData("reference_integrity")
      );
    ...
    function testReferenceIntegrity_deleteReferencedItem() {
      ...
      function onItem(item, request) {
        try {
          console.log(
            "Before delete map state is: " +
              dojo.toJson(item[store._reverseRefMap])
          );
          store.deleteItem(item);
          console.log(
            "After delete map state is: " +
              dojo.toJson(item[store._reverseRefMap])
          );
              ...
              for (var j = 0; j < attributes.length; j++) {
                ...
                for (var k = 0; k < values.length; k++) {
                  ...
                  try {
                    ...
                    /*Not an item, even a dead one, just eat it.*/
                  }
                }
                if (badRef) {
                  deferred.errback(
                    new Error(
                      "Found a reference remaining to a deleted item.  Failure."
                    )
                  );
                  passed = false;
                  break;
    ...
    function testReferenceIntegrity_deleteReferencedItemNonParent() {
      ...
      function onItem(item, request) {
        try {
          console.log(
            "Reference state for item 16 is: " +
              dojo.toJson(item[store._reverseRefMap])
          );
          store.deleteItem(item);
          function verifyRefDelete(items, request) {
    ...
    function testReferenceIntegrity_addReferenceToAttribute() {
      ...
      function onComplete(items, request) {
        ...
        //THIS IS FOR TESTING INTERNAL STATE!
        console.log(
          "Map state for Item 1 is: " + dojo.toJson(item1[store._reverseRefMap])
        );
        console.log(
          "Map state for Item 2 is: " + dojo.toJson(item2[store._reverseRefMap])
        );
        ...
        doh.assertTrue(
          item2[store._reverseRefMap][store.getIdentity(item1)]["siblings"]
        );
    ...
    function testReferenceIntegrity_newItemWithParentReference() {
      ...
      function onItem(item, request) {
        try {
          ...
          console.log(
            "State of map of item 17 after newItem: " + dojo.toJson(refs)
          );
        } catch (e) {
          console.debug(e);
          deferred.errback(e);
    ...
    function testReferenceIntegrity_newItemWithReferenceToExistingItem() {
      ...
      function onItem(item, request) {
        try {
          ...
          //THIS IS FOR TESTING INTERNAL STATE!
          console.log(
            "State of reference map to item 10 before newItem: " +
              dojo.toJson(item[store._reverseRefMap])
          );
          ...
          console.log(
            "State of reference map to item 10 after newItem: " +
              dojo.toJson(refs)
          );
        } catch (e) {
          console.debug(e);
    ...
    function testReferenceIntegrity_disableReferenceIntegrity() {
      ...
      var params = tests.data.ItemFileWriteStore.getTestData(
        "reference_integrity"
      );
      ...
      function onItem(item, request) {
        ...
        if (item[store._reverseRefMap] === undefined) {
          ...
          deferred.errback(
            new Error("Disabling of reference integrity failed.")
          );
        }
      }
      store.fetchItemByIdentity({
      ...
      if (dojo.isBrowser) {
        ...
        var onItem = function (item) {
          ...
          if (error === null) {
            d.errback(
              new Error(
                "Store was dirty, should have thrown an error on close!"
              )
            );
          } else {
            d.callback(true);
