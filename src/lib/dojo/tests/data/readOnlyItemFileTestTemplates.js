...
define([
  ...
  tests.data.readOnlyItemFileTestTemplates.registerTestsForDatastore =
    function (/* String */ datastoreClassName) {
      // summary:
      ...
      var makeNewTestFunction = function (template) {
        return function (t) {
          return template.runTest(datastoreClass, t);
        };
      };
  ...
  tests.data.readOnlyItemFileTestTemplates.testTemplates = [
      ...
      runTest: function (datastore, t) {
        ...
        if (dojo.isBrowser) {
          ...
          var completedAll = function (items, request) {
            d.errback(
              new Error("Should not be here, should have failed load.")
            );
          };
      ...
      runTest: function (datastore, t) {
        ...
        if (dojo.isBrowser) {
          ...
          function completedAll(items, request) {
            ...
            if (abortCalled) {
              console.log(
                "Made it to complete callback and abort was called.  Problem."
              );
              d.errback(new Error("Should not be here."));
            } else {
              //We beat out calling abort, so this is okay.  Timing.
              console.log(
                "in onComplete and abort has not been called.  Timing.  This is okay."
              );
              d.callback(true);
            }
          }
      ...
      runTest: function (datastore, t) {
        ...
        function onComplete(items, request) {
          ...
          if (!passed) {
            d.errback(
              new Error(
                "Located a child item with hierarchy off and no references in the data.  Error."
              )
            );
          } else {
            d.callback(true);
      ...
      runTest: function (datastore, t) {
        ...
        function onComplete(items, request) {
          ...
          if (!passed) {
            d.errback(
              new Error(
                "Found a non-child item in a reference list in a references based input.  Error."
              )
            );
          } else {
            d.callback(true);
    ...
    },
    {
      ...
      runTest: function (datastore, t) {
        ...
        var store = new datastore(
          tests.data.readOnlyItemFileTestTemplates.getTestData("data_multitype")
        );
      ...
      runTest: function (datastore, t) {
        ...
        function completed(items, request) {
          ...
          if (passed) {
            ...
            d.errback(
              new Error("Unexpected abbreviation found, match failure.")
            );
          }
        }
      ...
      runTest: function (datastore, t) {
        ...
        function completed(items, request) {
          ...
          if (passed) {
            ...
            d.errback(
              new Error("Unexpected pattern matched.  Filter failure.")
            );
          }
        }
        function error(error, request) {
      ...
      runTest: function (datastore, t) {
        ...
        function completed(items, request) {
          ...
          if (passed) {
            ...
            d.errback(
              new Error("Unexpected sorting order found, sort failure.")
            );
          }
        }
      ...
      runTest: function (datastore, t) {
        ...
        function completed(items, request) {
          ...
          for (var i = 0; i < items.length; i++) {
            var value = store.getValue(items[i], "value");
            if (
              !(items.length - (store.getValue(items[i], "uniqueId") + 1) === i)
            ) {
              passed = false;
              break;
            }
    ...
    },
    {
      ...
      runTest: function (datastore, t) {
        ...
        function completed(items, request) {
          var orderedArray = [
            0, 100, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000,
          ];
          t.assertEqual(items.length, 11);
          var passed = true;
          for (var i = 0; i < items.length; i++) {
            var value = store.getValue(items[i], "value");
            if (
              !(store.getValue(items[i], "value").getTime() === orderedArray[i])
            ) {
              passed = false;
              break;
            }
      ...
      runTest: function (datastore, t) {
        ...
        function completed(items, request) {
          ...
          for (var i = 0; i < items.length; i++) {
            ...
            if (
              !(
                store.getValue(items[i], "uniqueId") === orderedArray0[i] &&
    ...
    },
    {
      ...
      runTest: function (datastore, t) {
        ...
        function onItem(item) {
          ...
          t.assertTrue(
            dojo.date.stamp.toISOString(independenceDate, { zulu: true }) ===
