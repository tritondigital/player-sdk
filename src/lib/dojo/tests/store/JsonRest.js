define([
  ...
  "dojo/_base/lang",
  "dojo/store/JsonRest",
], function (doh, require, lang, JsonRest) {
  var globalHeaders = {
      ...
      function runTest(method, args) {
        ...
        store[method].apply(store, args).then(function (result) {
          ...
          for (k in requestHeaders) {
            if (
              !result.headers.hasOwnProperty(k) ||
              "" + result.headers[k] !== "" + requestHeaders[k]
            ) {
              error = true;
          ...
          for (k in globalHeaders) {
            if (
              !result.headers.hasOwnProperty(k) ||
              "" + result.headers[k] !== "" + globalHeaders[k]
            ) {
              error = true;
              d.errback(
                new Error("Global header mismatch in " + method + ": " + k)
              );
              return;
            }
          }
