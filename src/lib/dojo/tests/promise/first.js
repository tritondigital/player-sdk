define(["doh/main", "dojo/Deferred", "dojo/promise/first"], function (
  doh,
  Deferred,
  first
) {
  var tests = {
    "first() with array argument": function (t) {
      var obj = {};
      var received;
      first([this.deferred, new Deferred().resolve(obj), {}]).then(function (
        result
      ) {
        received = result;
      });
    ...
    "with object argument": function (t) {
      var obj = {};
      var received;
      first({ a: this.deferred, b: new Deferred().resolve(obj), c: {} }).then(
        function (result) {
          received = result;
        }
      );
    ...
    "first() with one rejected promise": function (t) {
      var obj = {};
      var received;
      first([this.deferred, new Deferred().reject(obj), {}]).then(
        null,
        function (result) {
          received = result;
        }
    ...
    "first() with one promise rejected later": function (t) {
      var obj = {};
      var received;
      first([this.deferred, new Deferred(), new Deferred()]).then(
        null,
        function (result) {
          received = result;
        }
    ...
    "first() with multiple promises rejected later": function (t) {
      ...
      var received;
      first([this.deferred, deferred2, new Deferred()]).then(
        null,
        function (result) {
          received = result;
        }
    ...
    "first() cancel only affects returned promise, not those we're waiting for":
      function (t) {
        var obj = {};
        var canceled = false;
        ...
        var received;
        var promise = first([this.deferred, deferred2, new Deferred()]).then(
          null,
          function (result) {
            received = result;
          }
