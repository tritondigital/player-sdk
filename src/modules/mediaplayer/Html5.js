...
define([
  ...
  var module = declare([techModule], {
    ...
    start: function () {
      ...
      this.html5Live.setTimeshiftInfoHandler(lang.hitch(this, this._onTimeshiftInfo));
    ...
    _onHlsCuePoint: function (data) {
      ...
      if (data.cuePoint && data.cuePoint.hlsTrackId) {
        ...
        if (audioTracks && audioTracks.length && audioTracks.length >= data.cuePoint.hlsTrackId && this._currentBitrate != audioTracks[data.cuePoint.hlsTrackId].bitRate) {
    ...
    _onAdBreak: function (data) {
      ...
      if (data.cuePoint != undefined) {
        ...
        data.adBreakData.duration =
          data.cuePoint[BreakCuePoint.CUE_TIME_DURATION] != undefined && data.cuePoint[BreakCuePoint.CUE_TIME_DURATION] != '' ? data.cuePoint[BreakCuePoint.CUE_TIME_DURATION] : null;
        data.adBreakData.isVastInStream =
          (data.cuePoint[BreakCuePoint.AD_VAST_URL] != undefined && data.cuePoint[BreakCuePoint.AD_VAST_URL] != '') ||
          (data.cuePoint[BreakCuePoint.AD_VAST] && data.cuePoint[BreakCuePoint.AD_VAST] != '')
            ? true
            : false;
    ...
    _isHTML5Supported: function () {
      ...
      if (this.html5CheckerNode != null) domConstruct.destroy(this.html5CheckerNode);
      return supported;
    }
  });
