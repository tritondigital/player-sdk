...
define([
    ...
    var aSyncCuePointDispatcher = declare([ ], {
        ...
        startCuePointsListener:function( mount )
        {
            ...
            this._npTimerId = setInterval( lang.hitch(this,  function(){ lang.hitch(this, this._getNowPlaying( this.mount ) ); }  ), this.POLLING_DELAY );
        },
            ...
            else
            {
                ...
                if( this._cuePoint.length > 0 )
                {
                    for (var i = 0; i < this._cuePoint.length; i++){
                        if( this._cuePoint[i].artist == artist && this._cuePoint[i].title == title )
                            return;
                    }
                }
        ...
        {
            if( this._cuePoint.length > 1 )
                this._cuePoint.shift();
        ...
        _onAdBreak:function( adTimestamp )
        {
            ...
            if( isAlreadySent == false )
                this._adBreakCuePointCallback( { mount:this.currentMount, cuePoint:this.currentCuePoint } );
        },
        ...
        _adBreakChecker:function( adTimestamp )
        {
            ...
            if( this._adBreaks.length > 2 )
                this._adBreaks.shift();
