...
function LiveStreamConfig(
  ...
  var endPoint = playerServicesRegion
    ? PROTOCOL +
      "//" +
      playerServicesRegion +
      "-" +
      platform.endpoint.liveStream
    : PROTOCOL + "//" + platform.endpoint.liveStream;
  var self = this;

  this.getStreamingConnections = function (
    query,
    mountTags,
    excludeUntagged,
    timeshift
    ...
    return liveStreamRepository
      .getLiveStreamConfig(query)
      .then(function (provisioningData) {
        var mountpoints = ArrayHelper.toSafeArray(
          provisioningData.mountpoints.mountpoint
        )
        ...
        mountpoints = AudioPriority.filterMountPoints(
          mountpoints,
          osPlatform,
          techType,
          mountTags,
      ...
      return _.flatMap(mountpoint.servers, function (server) {
        return mountpoint.transports.map(function (transport) {
          ...
          return {
            ...
            isHLS: transport.transport
              ? transport.transport.toLowerCase() === "hls" ||
                transport.transport.toLowerCase() === "hlsrw"
                ? true
                : false
              : false,
            isHLSTS: transport.transport
              ? transport.transport.toLowerCase() === "hlsts"
                ? true
                : false
              : false,
    ...
    if (isProgram) {
      ...
      return timeshift
        ? server +
            "/" +
            mountpoint.mount +
            (transport.mountSuffix ? transport.mountSuffix : "")
        : server +
            "/" +
            mountpoint.mount +
            (transport.mountSuffix ? transport.mountSuffix : "") +
            (techType.toLowerCase() === "html5" ? extension : "");
    }
  }

  function getExtension(mountpoint) {
    return mountpoint.mediaFormat.audioTracks[0].isMP3
      ? ".mp3"
      : simpleAAC(mountpoint);
  }

  function simpleAAC(mountpoint) {
    return (mountpoint.mediaFormat.audioTracks[0].isAAC &&
      !mountpoint.isAudioAdaptive &&
      !mountpoint.hasHLS() &&
      !mountpoint.hasHLSTS()) ||
      (mountpoint.mediaFormat.audioTracks[0].isAAC &&
        !forceHls &&
        !forceHlsts &&
        osPlatform.os.family.toLowerCase() == "android")
      ? ".aac"
      : "";
  }
