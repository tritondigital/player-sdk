...
function MountPoint( mountPointData ) {
	...
	function parse( context, data ) {
		...
		if ( data.status[ 'status-code' ]._text < 300 ) {
			...
			context.tags = ( !data._attr || data._attr.tags || data._attr.tags._value === '' ) ? [] : data._attr.tags._value.split( ',' ).map( function ( tag ) {
				return new Tag( tag );
			} );
			...
			context.metrics = ( !data.metrics || _.isEmpty( data.metrics.tag ) ) ? [] : ArrayHelper.toSafeArray( data.metrics.tag ).map( function ( metricTag ) {
	...
	// 	} else {
	// 		var br = 0;
	// 		array.forEach( this.format().audioTracks().concat( this.format().videoTracks() ), function ( track ) {
	// 			br += track.bitRate();
	// 		} );
	// 		return br;
	// 	}
