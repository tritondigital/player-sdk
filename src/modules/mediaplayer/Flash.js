...
define([
...
], function (declare, lang, dom, domAttr, TrackCuePoint, BreakCuePoint, HlsCuePoint, errorCode, techModule, domConstruct, topic, AdManager, Html5OnDemand) {
  var module = declare([techModule], {
    ...
    resumeStream: function () {
      ...
      if (typeof this.flashMovie.tdPlayer_resumeStream == 'function') this.flashMovie.tdPlayer_resumeStream();
    },

    seekStream: function (seekOffset) {
      this.inherited(arguments);

      if (typeof this.flashMovie.tdPlayer_seekStream == 'function') this.flashMovie.tdPlayer_seekStream(seekOffset);
    },

    seekLive: function () {
      this.inherited(arguments);

      if (typeof this.flashMovie.tdPlayer_seekLive == 'function') this.flashMovie.tdPlayer_seekLive();
    },
    ...
    mute: function () {
      ...
      if (typeof this.flashMovie.tdPlayer_mute == 'function') this.flashMovie.tdPlayer_mute();
    },

    unMute: function () {
      this.inherited(arguments);

      if (typeof this.flashMovie.tdPlayer_unMute == 'function') this.flashMovie.tdPlayer_unMute();
    },

    skipAd: function () {
      this.inherited(arguments);

      if (typeof this.flashMovie.tdPlayer_skipAd == 'function') this.flashMovie.tdPlayer_skipAd();
    },
    ...
    pauseMedia: function () {
      ...
      if (typeof this.flashMovie.tdPlayer_pauseMedia == 'function') this.flashMovie.tdPlayer_pauseMedia();
    },

    resumeMedia: function () {
      this.inherited(arguments);

      if (typeof this.flashMovie.tdPlayer_resumeMedia == 'function') this.flashMovie.tdPlayer_resumeMedia();
    },
    ...
    seekMedia: function (seekOffset) {
      ...
      if (typeof this.flashMovie.tdPlayer_seekMedia == 'function') this.flashMovie.tdPlayer_seekMedia(seekOffset);
    },
    ...
    _onScriptData: function (data) {
      ...
      if (data.onCuePoint) {
        ...
        if (cuePoint.parameters.tag_timestamp) {
          cuePoint.parameters.tag_timestamp_formatted = this.toHHMMSS(data.tagTimestamp / 1000);
        }
        ...
        cuePoint.parameters.time_played = moment(new Date().getTime() + delay).format('HH:mm:ss.SSS');

        if (cuePoint.type == 'ad') {
          var adBreakData = {};
          adBreakData.type = 'ad';
          adBreakData.tagTimestamp = data.tagTimestamp;
          adBreakData.tagTimestampFormatted = this.toHHMMSS(data.tagTimestamp / 1000);
          adBreakData.timePlayed = cuePoint.parameters.time_played;
          adBreakData.adVast = cuePoint[BreakCuePoint.AD_VAST] != undefined && cuePoint[BreakCuePoint.AD_VAST] != '' ? cuePoint[BreakCuePoint.AD_VAST] : null;
          adBreakData.adType = cuePoint[BreakCuePoint.AD_TYPE] != undefined && cuePoint[BreakCuePoint.AD_TYPE] != '' ? cuePoint[BreakCuePoint.AD_TYPE] : null;
          adBreakData.cueID = cuePoint[BreakCuePoint.AD_ID] != undefined && cuePoint[BreakCuePoint.AD_ID] != '' ? cuePoint[BreakCuePoint.AD_ID] : null;
          adBreakData.cueTitle = cuePoint[BreakCuePoint.CUE_TITLE] != undefined && cuePoint[BreakCuePoint.CUE_TITLE] != '' ? cuePoint[BreakCuePoint.CUE_TITLE] : null;
          adBreakData.duration = cuePoint[BreakCuePoint.CUE_TIME_DURATION] != undefined && cuePoint[BreakCuePoint.CUE_TIME_DURATION] != '' ? cuePoint[BreakCuePoint.CUE_TIME_DURATION] : null;
          adBreakData.isVastInStream =
            (cuePoint[BreakCuePoint.AD_VAST_URL] != undefined && cuePoint[BreakCuePoint.AD_VAST_URL] != '') || (cuePoint[BreakCuePoint.AD_VAST] && cuePoint[BreakCuePoint.AD_VAST] != '')
              ? true
              : false;
          adBreakData.url = cuePoint[BreakCuePoint.AD_URL] != undefined && cuePoint[BreakCuePoint.AD_URL] != '' ? cuePoint[BreakCuePoint.AD_URL] : null;
          adBreakData.vastUrl = cuePoint[BreakCuePoint.AD_VAST_URL] != undefined && cuePoint[BreakCuePoint.AD_VAST_URL] != '' ? cuePoint[BreakCuePoint.AD_VAST_URL] : null;

          setTimeout(() => {
            if (adBreakData.isVastInStream == true && (adBreakData.adVast || adBreakData.vastUrl)) {
    ...
    __createMediaTag: function (mediaTagType) {
      ...
      var node = domConstruct.create(mediaTagType, { id: 'tdplayer_flv_' + mediaTagType + 'node', type: mediaTagType }, this.flvNode, 'first');
      domAttr.set(node, 'height', '0%');
      domAttr.set(node, 'width', '0%');
      domAttr.set(node, 'x-webkit-airplay', 'allow'); //AirPlay support
      var sourceNode = domConstruct.create('source', { id: 'tdplayer_flv' + mediaTagType + 'source' }, node, 'first');
    ...
    _createPlayer: function (url) {
      ...
      this.player.load();
      this.player.play();
      this.player.on(flvjs.Events.METADATA_ARRIVED, lang.hitch(this, this._onMetadata));

      this.player.on(flvjs.Events.STREAMDATA_ARRIVED, lang.hitch(this, this._onStreamData));

      this.player.on(flvjs.Events.ERROR, lang.hitch(this, this._onError));

      this.player.on(flvjs.Events.MEDIA_INFO, e => console.log('Media Info Arrived:' + JSON.stringify(e)));

      this.player.on(flvjs.Events.SCRIPTDATA_ARRIVED, lang.hitch(this, this._onScriptData));
    ...
    _embedFlash: function () {
      ...
      if (this.config.plugins != undefined && this.config.plugins.length > 0) corePlugins = corePlugins.concat(this.config.plugins);
      ...
      if (this.config.moduleDir != null) playerConfig.playercore.module_dir = this.config.moduleDir;
      ...
      if (this.flvNode == null) {
        this.flvNode = domConstruct.create('div', { id: 'tdplayer_flv' }, this.playerNode, 'first');
      }
