    ...
    var artist = declare([ Inpe ], {
        ...
        getAlbums:function()
        {
            if ( this.albums == null )
            {
                ...
                var album;
                array.forEach(this.artistData.albums, function(item, index){
        ...
        {
            var itemIndex = -1;
            array.forEach(this.getAlbums(), function(item, index){
                if (item.id == albumId)
                    itemIndex = index;
            },this);
        ...
        getPictures:function()
        {
            if ( this.pictures == null )
            {
                ...
                var picture;
                array.forEach(this.artistData.pictures, function(item, index){
        ...
        {
            var itemIndex = -1;
            array.forEach(this.getPictures(), function(item, index){
        ...
        fetchData:function(isDynamicCall)
        {
            ...
            if ( isDynamicCall )
                this.url = this.getDynamicArtistUrl( this.id );
        ...
        fetchAlbumByIds:function( albumIds )
        {
            ...
            for ( var i=0; i < albumIds.length; i++ )
            {
                ...
                if (album.successCallback)
                    album.successCallback.remove();
                if (album.errorCallback)
                    album.errorCallback.remove();
        ...
        fetchPictureByIds:function( pictureIds )
        {
            ...
            for ( var i=0; i < pictureIds.length; i++ )
            {
                ...
                if (picture.successCallback)
                    picture.successCallback.remove();
                if (picture.errorCallback)
                    picture.errorCallback.remove();
        ...
        _removeAlbumFromQueue:function( albumId )
        {
            ...
            array.forEach(this.albumsQueue, function( itemId, index ){
                if ( itemId == albumId )
                    spliceStartIndex = index;
            },this);

            if ( spliceStartIndex > -1 )
                this.albumsQueue.splice(spliceStartIndex, 1);
        },
        ...
        _removePictureFromQueue:function( pictureId )
        {
            ...
            var spliceStartIndex = -1;
            array.forEach(this.picturesQueue, function( itemId, index ){
                if ( itemId == pictureId )
                    spliceStartIndex = index;
            },this);

            if ( spliceStartIndex > -1 )
                this.picturesQueue.splice(spliceStartIndex, 1);
        }
