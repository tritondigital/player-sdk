    ...
    var album = declare([ Inpe ], {
        ...
        getCoverArtOriginal:function()
        {
            ...
            if( cover != null || cover != undefined )
                return { url:cover.originalSourceUrl, width:cover.width, height:cover.height, id:cover.id };
            else
                return null;
        },
        ...
        getTracks:function()
        {
            if ( this.tracks == null )
            {
                ...
                var track;
                array.forEach(this.albumData.tracks, function(item, index){
        ...
        {
            var itemIndex = -1;
            array.forEach(this.getTracks(), function(item, index){
                if (item.id == trackId)
                    itemIndex = index;
            },this);
        ...
        fetchData:function(isDynamicCall)
        {
            ...
            if ( isDynamicCall )
                this.url = this.getDynamicAlbumUrl( this.id );
        ...
        fetchTrackByIds:function( trackIds )
        {
            ...
            for ( var i=0; i < trackIds.length; i++ )
            {
                ...
                if (track.successCallback)
                    track.successCallback.remove();
                if (track.errorCallback)
                    track.errorCallback.remove();
        ...
        _removeTrackFromQueue:function( trackId )
        {
            ...
            array.forEach(this.tracksQueue, function( itemId, index ){
                if ( itemId == trackId )
                    spliceStartIndex = index;
            },this);

            if ( spliceStartIndex > -1 )
                this.tracksQueue.splice(spliceStartIndex, 1);
        }
