...
            }
        ...
        _onMediaAdReady: function( e ) {
            ...
            if( mediaFile != null )
            {
                this.tech.html5OnDemand.playAd( { adServerType: data.adServerType, mediaUrl:mediaFile.url, mediaFormat:mediaFile.format, clickThrough:data.clickThrough, clickTrackings:data.clickTrackings } );
            ...
            switch ( data.adServerType )
            {
                ...
                case AdServerType.MEDIA_AD :
                    ...
                    for ( var i = 0; i < mediaFilesLength; i++ )
                    {
                        ...
                        if( ( mediaFiles[i].type == 'audio/aac' || mediaFiles[i].url.split('.').pop() == 'aac' ) && this._isTypeSupported('audio', 'audio/aac;') )
                            ...
                            return { url:mediaFiles[i].url, format:'audio' };
                        else if( ( mediaFiles[i].type == 'audio/webm' ) && this._isTypeSupported('audio', 'audio/webm;') )
                            return { url:mediaFiles[i].url, format:'audio' };
                        else if( ( mediaFiles[i].type == 'video/mp4' || mediaFiles[i].url.split('.').pop() == 'mp4' || mediaFiles[i].url.split('.').pop() == 'm4v' ) && this._isTypeSupported('video', 'video/mp4;') ){
