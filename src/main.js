...
window.TDSdk = declare( [], {
	...
	_parseConfig: function ( playerConfig, isBlocked ) {
		...
		if ( filteredArr.length && !isBlocked ) {
			...
			array.forEach( syncBanners.elements, function ( element ) {
		...
		if ( filteredArr.length ) {
			...
			mediaPlayer.defaultTrackingParameters.banners = ( mediaPlayer.defaultTrackingParameters.banners != undefined ) ? this._unique( mediaPlayer.defaultTrackingParameters.banners.concat( banners ) ) : banners;
	...
	_onModuleReady: function ( e ) {
		...
		if ( e.data.module.api )
			declare.safeMixin( this, e.data.module.api );
