...
describe("VAST2Parser", function () {
  ...
  it("should return companions with 5 tracking events", function () {
    expect(
      vastDocument.vastAd.inlineAd.creatives[1].companionAds[3].creativeView
    ).to.have.length(6);
  });

  it("should return companions with 1 tracking events", function () {
    expect(
      vastDocument.vastAd.inlineAd.creatives[1].companionAds[0].creativeView
    ).to.have.length(1);
  });

  it("should return NULL on companions without tracking events", function () {
    expect(
      vastDocument.vastAd.inlineAd.creatives[1].companionAds[1].creativeView
    ).to.be(null);
  });
});
